...
FROM
(
SELECT CAST(userId AS BIGINT) as userId, firstName, lastName, gender, dateOfBirth, level, CAST(registration as BIGINT) as registration, minDate as rowActivationDate,
-- Choose the start date from the next record and add that as the expiration date for the current record
LEAD(minDate, 1, '9999-12-31') OVER(PARTITION BY userId, firstName, lastName, gender, dateOfBirth ORDER BY grouped) as rowExpirationDate,
...
SELECT userId, firstName, lastName, gender, dateOfBirth, registration, level, grouped, cast(min(date) as date) as minDate
FROM
...
SELECT CAST(userId as BIGINT) as userKey, firstName, lastName, gender, dateOfBirth, level, CAST(registration as BIGINT) as registration, CAST(min(ts) as date) as rowActivationDate, DATE '9999-12-31' as rowExpirationDate, 1 as currentRow
FROM {{ source('staging', 'listen_events') }} 
